...
Vue.component('account-edit-amount-form', {
    ...
    template:
        '<div class="editForm" :id="`amount_form`+amount.id">' +
        'New amount:' +
...
Vue.component('account-edit-name-form', {
    props: ['account', 'accounts'],
    data: function() {
        return {
            name: this.account.name,
    ...
    template:
        '<div class="editForm" :id="`name_form`+amount.id">' +
        'New account name:' +
...
Vue.component('account-row', {
    ...
    template:
        '<div >' +
        '<account-edit-name-form style="display: none; z-index: 9999; position: absolute;" :id="`name_form`+account.id" :accounts="accounts" :account="account" />' +
        '<account-edit-amount-form style="display: none; z-index: 9999; position: absolute;" :id="`amount_form`+account.id" :accounts="accounts" :account="account" />' +
        ...
        '<input class="button" :id="`editName`+account.id" type="button" value="Edit" @click="askEditName" />' +
        '<input class="button" :id="`editAmount`+account.id" type="button" value="Check" @click="askEditAmount" />' +
        '<input class="button" :id="`delete`+account.id" type="button" value="X" @click="del" />' +
        '</div>' +
        '</div>',
    methods: {
        askEditName: function() {
            const form = document.getElementById('name_form'+this.account.id);
